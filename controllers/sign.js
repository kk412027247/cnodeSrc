...
exports.signin=function(req,res){
	...
	//把登陆信息存在session里面，存在redis里面
	UserModel.getUser(username,pass,function(err,user){
		if(user){
			//这里把登陆的时候查询到的数据对象全部传给了session.user
			req.session.user=user;
			res.render('sign/signin',{success:'登陆成功',login:1})
		}else{
			res.status(422);
			res.render('sign/signin',{error:'用户名和密码错误'})
		}
	})
}
